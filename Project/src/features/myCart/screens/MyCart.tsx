  ...
  return (
    <SafeAreaView style={styles.container}>
      {data.cart.cartItems.length !== 0 ? (
        <FlatList data={data.cart.cartItems} 
        showsVerticalScrollIndicator={false}
        renderItem={ ({item}) => 
        <MyCartItem cartItem={item} onRemoveItemPress={() => removeItem(item.id)}/>
      }
        ListFooterComponent={
              <Surface style={styles.priceDetailcontainer}>
          <BaseTitleText text='Price details' textStyle={{textAlign: 'left'}} containerStyle={{alignItems: 'flex-start', marginBottom: 20}}/>
          <PriceDetailItem title='Price' value={data.cart.price}/>
          <PriceDetailItem title='Delivery' value={data.cart.deliveryPrice}/>
          <PriceDetailItem title='Discount' value={data.cart.discount} isDiscount={true}/>
        ...
        <BaseButton text='proceed to payment' style={{marginBottom: 30}} onPress={makeOrder}/>
    </SafeAreaView>
  );
};
